...
int insereNoAVL(avl *arv, int valor) {
  ...
  if (novo != arv->sentinela->Fdir) {
    atualizaFB_insercaoAVL(arv, novo);
  }
  return 1;
}
...
int removeNoAVL(avl *arv, int valor) {
  ...
  if (atual->Fesq == NULL) {
    ...
    atualizaFB_remocaoAVL(arv, sucessor->pai, sucessor->chave);
    free(sucessor);
    return 1;
  }

  atualizaFB_remocaoAVL(arv, atual->pai, atual->chave);
  free(atual);
  return 1;
}
...
int processaCargaAVL(avl *arv, char *nomeArquivo, int tipo) {
  ...
  if (tipo == 1) {
    while (fscanf(arquivo, "%d", &valor) != EOF) {
      res = insereNoAVL(arv, valor);
      if (res != 1)
        return 0;
    }
  } else {
    while (fscanf(arquivo, "%d", &valor) != EOF) {
      res = removeNoAVL(arv, valor);
      if (res != 1)
        return 0;
    }
  }
